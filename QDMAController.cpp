...
CPUMemCtl::CPUMemCtl(uint64_t size, int hfd_id) {
    pool_size = size;
    ...
    std::string hfd_path = fmt::format("/media/huge/hfd{}", hfd_id);
...
CPUMemCtl *CPUMemCtl::getInstance(size_t pool_size, int hfd_id) { // hfd_id is used when multiple processes need to allocate their own CPU memory pool.
    // up round to 2MB
    pool_size = (pool_size + 2UL * 1024 * 1024 - 1) & ~(2UL * 1024 * 1024 - 1);

    if (cpu_mem_ctl_list.empty()) {
        auto tmp = new CPUMemCtl(pool_size, hfd_id);
